...
public class BodyTalkController {
    ...
    @SendTo("/topic/game/{roomId}/bodyTalk/keyword")
    public GameMessage<String> sendKeyword(@DestinationVariable("roomId") String roomId) {
        log.info("========= 몸으로 말해요 게임 키워드 전달 =========");
        String type = bodyTalkService.decideKeywordExplanier(roomId);
        return GameMessage.info("게임 키워드 타입 전달 성공",type);
    }
